...
{
	u8 needed_ammo_type = (m_set_next_ammoType_on_reload != u8(-1) && m_ammoType != m_set_next_ammoType_on_reload) ? m_set_next_ammoType_on_reload : m_ammoType;
	bool b_single_bullet = ((m_magazine.size() == 1 || !HaveCartridgeInInventory(2, m_ammoType)) && (m_set_next_ammoType_on_reload == u32(-1) || m_ammoType == m_set_next_ammoType_on_reload));
	bool isAmmoChange = m_magazine.size() > 0 && m_set_next_ammoType_on_reload != u8(-1) && m_ammoType != m_set_next_ammoType_on_reload;
	...
	if (IsMisfire())
	{
		...
		strconcat(sizeof(sndUnmisName), sndUnmisName, "sndReloadMisfire", std::to_string(m_magazine.size()).c_str());
...
void CWeaponBM16::PlayAnimReload()
{
	...
	if (IsMisfire())
	{
		...
		strconcat(sizeof(anmUnmisName), anmUnmisName, isHUDAnimationExist("anim_reload", true) ? "anim_reload" : "anm_reload", isHUDAnimationExist("anm_reload_misfire", true) ? "_misfire" : "_jammed", "_", std::to_string(m_magazine.size()).c_str());

		if (isHUDAnimationExist(anmUnmisName))
		{
			PlayHUDMotionIfExists({ anmUnmisName, b_single_bullet ? "anim_reload_1" : "anim_reload", b_single_bullet ? "anm_reload_1" : "anm_reload_2" }, true, GetState());
			return;
		}
	}
	...
	if (isAmmoChange)
	{
		...
		{
			PlayHUDMotionIfExists({ anmAmmoChangeName, b_single_bullet ? "anim_reload_1" : "anim_reload", b_single_bullet ? "anm_reload_1" : "anm_reload_2" }, true, GetState());
			return;
		}
	}
	...
	PlayHUDMotionIfExists({ anmReloadName, b_single_bullet ? "anim_reload_1" : "anim_reload", b_single_bullet ? "anm_reload_1" : "anm_reload_2" }, true, GetState());
}
