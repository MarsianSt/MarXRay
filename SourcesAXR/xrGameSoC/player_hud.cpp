...
u32 attachable_hud_item::anim_play(const shared_str& anm_name_b, BOOL bMixIn, const CMotionDef*& md, u8& rnd_idx, float speed)
{
	...
	IKinematicsAnimated* ka = m_model->dcast_PKinematicsAnimated();
	u32 ret					= g_player_hud->anim_play(m_attach_place_idx, anm->params, M.mid, bMixIn, md, speed, m_has_separated_hands, ka);
...
#pragma todo("DANCE MANIAC: Need Edit for SoC hands.")
	return motion_length(phm->params, phm->m_animations[0].mid, temp, speed, m_model);
}
...
u32 player_hud::motion_length(const shared_str& anim_name, const shared_str& hud_name, const CMotionDef*& md, float speed)
{
	...
	return motion_length			(pm->params, pm->m_animations[0].mid, md, speed == 1.f ? pm->params.speed_k : speed, pi->m_has_separated_hands ? m_model : smart_cast<IKinematicsAnimated*>(pi->m_model));
}

u32 player_hud::motion_length(const motion_params& P, const MotionID& M, const CMotionDef*& md, float speed, IKinematicsAnimated* itemModel)
{
...
u32 player_hud::script_anim_play(u8 hand, LPCSTR section, LPCSTR anm_name, bool bMixIn, float speed, LPCSTR attach_visual)
{
	...
	u32 length = motion_length(phm->params, M.mid, md, speed, m_model);
...
u32 player_hud::anim_play(u16 part, const motion_params& P, const MotionID& M, BOOL bMixIn, const CMotionDef*& md, float speed, bool hasHands, IKinematicsAnimated* itemModel, u16 override_part)
{
	if (hasHands)
	{
		u16 part_id = u16(-1);
	...
	u32 length = motion_length(P, M, md, speed, hasHands ? m_model : itemModel);
