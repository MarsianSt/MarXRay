...
void CUIMultiElement::InitFromXml(CUIXml& xml, LPCSTR path, int index, CUIWindow* pWnd, bool pos_by_parent)
{
	...
	if (m_static = UIHelper::CreateStatic(xml, path, this, false))
	{
		...
		{
			m_value = UIHelper::CreateStatic(xml, "value", m_static);
...
void CUIMultiElement::SetColorAnimation(LPCSTR lanim, bool bCyclic, bool bOnlyAlpha, bool bTextColor, bool bTextureColor)
{
	if (m_static)
		m_static->SetClrLightAnim(lanim, bCyclic, bOnlyAlpha, bTextColor, bTextureColor);

	if (m_value)
		m_value->SetClrLightAnim(lanim, bCyclic, bOnlyAlpha, bTextColor, bTextureColor);

	if (m_progress)
		m_progress->SetColorAnimation(lanim, bCyclic, bOnlyAlpha, bTextColor, bTextureColor);

	/*if (m_progress_shape)
		m_progress_shape->SetClrLightAnim(lanim, bCyclic, bOnlyAlpha, bTextColor, bTextureColor);

	if (m_arrow)
		m_arrow->SetClrLightAnim(lanim, bCyclic, bOnlyAlpha, bTextColor, bTextureColor);

	if (m_arrow_shadow)
		m_arrow_shadow->SetClrLightAnim(lanim, bCyclic, bOnlyAlpha, bTextColor, bTextureColor);*/
}

void CUIMultiElement::ResetColorAnimation()
{
	if (m_static)
		m_static->ResetClrAnimation();

	if (m_value)
		m_value->ResetClrAnimation();

	if (m_progress)
		m_progress->ResetColorAnimation();

	/*if (m_progress_shape)
		m_progress_shape->ResetClrAnimation();

	if (m_arrow)
		m_arrow->ResetClrAnimation();

	if (m_arrow_shadow)
		m_arrow_shadow->ResetClrAnimation();*/
}
