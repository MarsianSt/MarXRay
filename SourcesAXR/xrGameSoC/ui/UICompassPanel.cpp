...
bool CUICompassPanel::Init()
{
	...
	if (uiXml.NavigateToNode("compass_panel:point_dist", 0))
	{
		...
		m_point_dist->SetText("");
		m_point_dist->SetTextAlignment(CGameFont::alCenter);
		m_point_dist->SetVTextAlignment(valCenter);
	...
	if (uiXml.NavigateToNode("compass_panel:point_name", 0))
	{
		...
		m_point_name->SetText("");
		m_point_name->SetTextAlignment(CGameFont::alCenter);
		m_point_name->SetVTextAlignment(valCenter);
...
void CUICompassPanel::InitDirections(CUIXml& uiXml)
{
	...
	for (int i = 0; i < DIRECTION_COUNT; i++)
	{
		...
		if (m_directions[i].enabled)
		{
			...
			st->SetText(m_directions[i].name.c_str());
			st->SetTextColor(m_directions[i].color);
			st->SetFont(m_directions[i].font);
...
void CUICompassPanel::AddPoints()
{
	...
	for (const auto& map_loc : Level().MapManager().Locations())
	{
		...
		CGameTask* active_task = Actor()->GameTaskManager().ActiveTask();
		SGameTaskObjective* active_objective = Actor()->GameTaskManager().ActiveObjective();

		bool is_quest = (active_objective && active_objective->LinkedMapLocation() && map_location->ObjectID() == active_objective->LinkedMapLocation()->ObjectID());
		...
		if (enabled)
		{
			...
			if (is_quest)
			{
				CSE_ALifeDynamicObject* main_target_obj = nullptr;
				CSE_ALifeDynamicObject* additional_target_obj = nullptr;
				...
				if (active_objective)
				{
					u16 target_id = active_objective->GetMapObjectID();
					if (target_id != u16(-1))
					{
						main_target_obj = ai().alife().objects().object(target_id, true);
						map_location = active_objective->LinkedMapLocation();
					}
				}

				CGameTask* task = Actor()->GameTaskManager().FindTaskByMapLocation(map_location);

				if (!task)
					continue;

				u16 map_object_id = Actor()->GameTaskManager().GetTaskTargetObjectID(task, map_location);
				...
				if (active_objective && active_task && task->m_ID == active_task->m_ID)
				{
					alife_obj = main_target_obj;
					is_active = true;
				}
				...
				if (map_location->LevelName() == Level().name())
				{
					AddQuestPoint(map_location->ObjectID(), alife_obj, *CStringTable().translate(task->m_Title), map_location, is_active);
				}
				else
				{
			...
			{
				bool is_suitable_lc = level_changer && (level_changer->GetNextLevelName() == map_location->LevelName());
	...
	for (auto& point : m_points)
	{
		...
		if (point.spot_icons[1]->inited() && point.spot_icons[2]->inited())
		{
			...
			if (d > 1.8f)
			{
				...
				st->GetUIStaticItem().SetOriginalRect(point.tex_rects[2].x1, point.tex_rects[2].y1, point.tex_rects[2].width(), point.tex_rects[2].height());
			}
			else
			{
				if (d < -1.8f)
				{
					...
					st->GetUIStaticItem().SetOriginalRect(point.tex_rects[1].x1, point.tex_rects[1].y1, point.tex_rects[1].width(), point.tex_rects[1].height());
				}
				else
				{
					st->SetShader(point.spot_icons[0]);
					st->GetUIStaticItem().SetOriginalRect(point.tex_rects[0].x1, point.tex_rects[0].y1, point.tex_rects[0].width(), point.tex_rects[0].height());
				}
			}
		}
...
void CUICompassPanel::ShowPointInfo(const SCompassPoint& point, string128 dist_str, Fvector2 text_pos, u32 alpha_color, bool show_text)
{
	if (m_point_dist)
	{
		m_point_dist->SetText(dist_str);
		m_point_dist->SetTextColor(alpha_color);
	...
	if (m_point_name)
	{
		...
		{
			m_point_name->SetText(point.text.c_str());
			m_point_name->SetTextColor(alpha_color);
