...
void CGamePersistent::WeathersUpdate()
{
	...
	else
	{
		if (g_pGameLevel)
		{
			...
			if (actor)
				bIndoor = g_pGamePersistent->IsActorInHideout() && (actor->renderable_ROS()->get_luminocity_hemi() < 0.04f); //--#SM+#-- [0.05f слишком завышен был]
			...
			if (env_amb)
			{
				...
				if ((FALSE == bIndoor) && (0 == ambient_particles) && Device.dwTimeGlobal > ambient_effect_next_time)
				{
					...
					if (eff)
					{
						...
						ambient_particles = CParticlesObject::Create(eff->m_particles.c_str(), FALSE, false);
						Fvector pos;
						pos.add(Device.vCameraPosition, eff->m_offset);
						ambient_particles->play_at_pos(pos);
						if (eff->m_sound._handle())
							eff->m_sound.play_at_pos(0, pos);
					}
				}
...
void CGamePersistent::OnFrame	()
{
	...
	if(Device.Paused())
	{
		...
		if (Level().CurrentViewEntity() && IsGameTypeSingle()) {
			if (!g_actor || (g_actor->ID() != Level().CurrentViewEntity()->ID())) {				CCustomMonster	*custom_monster = smart_cast<CCustomMonster*>(Level().CurrentViewEntity());
