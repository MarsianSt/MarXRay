...
//	Msg("%d && %d && (%d || %d) && (%d || %d)", !m_magazine.empty(), fTime<=0, IsWorking(), m_bFireSingleShot, m_iQueueSize < 0, m_iShotNum < m_iQueueSize);
	while (!m_magazine.empty() && fTime <= 0 && (IsWorking() || m_bFireSingleShot) && (m_iQueueSize < 0 || m_iShotNum < m_iQueueSize))
	{
		...
		VERIFY			(fTimeToFire>0.f);
		fTime			+=	b_mod_shot_time ? fModeShotTime : b_fast_mode ? fFastShotTime : fTimeToFire;
		fTime			+= ((b_mod_shot_time ? fModeShotTime : b_fast_mode ? fFastShotTime : fTimeToFire) * (m_fOverheatingSubRpm / 100.f)) * m_fWeaponOverheating;
	...
	// Эффект сдвига (отдача)
	{
		...
		if (m_b_advanced_shoot_effectors && IsGameTypeSingle() && ParentIsActor())
		{
			...
			if (pSettings->section_exist(effector_sect))
			{
				...
				float disp = ((GetFireDispersion(m_fCurrentCartirdgeDisp) * 150.f) * (IsZoomed() ? effector_intensity_aim : Actor()->is_actor_crouch() ? effector_intensity_crouch : effector_intensity)) * m_b_advanced_se_factor;
...
void CWeaponMagazined::switch2_Unmis()
{
	...
	if (isHUDAnimationExist(anmUnmisName))
		PlayHUDMotionIfExists({ anmUnmisName, "anm_reload", "anim_reload" }, true, GetState());
	else
		PlayAnimReload();
}
...
void CWeaponMagazined::PlayAnimShoot()
{
	...
	if (iAmmoElapsed == 1)
		PlayHUDMotionIfExists({ guns_shoot_anm, "anm_shoot_last", "anm_shots_last", "anm_shot_l", "anm_shoot", "anim_shoot", "anm_shots" }, false, GetState());
	else
		PlayHUDMotionIfExists({ guns_shoot_anm, "anm_shoot", "anim_shoot", "anm_shots" }, false, GetState());
}
