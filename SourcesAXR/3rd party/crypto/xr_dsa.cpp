...
{
	LogInfo("* dsa genparams cb(%d, %d)", p, n);
}
...
void xr_dsa::generate_params()
{
	...
	LogInfo("u8 const p_number[crypto::xr_dsa::public_key_length] = {");
	print_big_number	(tmp_dsa_params->p);
	LogInfo("};//p_number");

	
	LogInfo("u8 const q_number[crypto::xr_dsa::private_key_length] = {");
	print_big_number	(tmp_dsa_params->q);
	LogInfo("};//q_number");
	
	
	LogInfo("u8 const g_number[crypto::xr_dsa::public_key_length] = {");
	print_big_number	(tmp_dsa_params->g);
	LogInfo("};//g_number");

	LogInfo("u8 const public_key[crypto::xr_dsa::public_key_length] = {");
	...
	{
		LogInfo("	m_private_key.m_value[%d]	= 0x%02x;", i, priv_bin[i]);
	}
