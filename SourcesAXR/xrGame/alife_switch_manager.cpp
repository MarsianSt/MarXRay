...
void CALifeSwitchManager::add_online(CSE_ALifeDynamicObject *object, bool update_registries)
{
	...
	{
		LogInfo("Trying to correct invalid vertex %u for object %s", object->m_tNodeID,
			object->name_replace());
		object->m_tNodeID = ai().level_graph().vertex(object->m_tNodeID, object->o_Position);
		LogInfo("  new vertex: %u", object->m_tNodeID);
	}
	...
	if (psAI_Flags.test(aiALife))
		LogInfo("[LSS] Spawning object [%s][%s][%d]",object->name_replace(),*object->s_name,object->ID);
#endif
...
#ifdef DEBUG
	if (psAI_Flags.test(aiALife))
		LogInfo("[LSS] Destroying object [%s][%s][%d]",object->name_replace(),*object->s_name,object->ID);
#endif
...
#ifdef DEBUG
//	if (psAI_Flags.test(aiALife))
		LogInfo("[LSS][%d] Going online [%d][%s][%d] ([%f][%f][%f] : [%f][%f][%f]), on '%s'",Device.dwFrame,Device.dwTimeGlobal,object->name_replace(), object->ID,VPUSH(graph().actor()->o_Position),VPUSH(object->o_Position), "*SERVER*");
#endif
...
#ifdef DEBUG
//	if (psAI_Flags.test(aiALife))
		LogInfo("[LSS][%d] Going offline [%d][%s][%d] ([%f][%f][%f] : [%f][%f][%f]), on '%s'",Device.dwFrame,Device.dwTimeGlobal,object->name_replace(), object->ID,VPUSH(graph().actor()->o_Position),VPUSH(object->o_Position), "*SERVER*");
#endif
	...
	if (0xffff != I->ID_Parent) {
		...
		if (psAI_Flags.test(aiALife)) {
			...
			if (l_tpALifeCreatureAbstract && (l_tpALifeCreatureAbstract->get_health() < EPS_L))
				LogInfo("! uncontrolled situation [%d][%d][%s][%f]",I->ID,I->ID_Parent,l_tpALifeCreatureAbstract->name_replace(),l_tpALifeCreatureAbstract->get_health());
