...
void clientdata_proxy::make_screenshot(ClientID const & admin_id, ClientID const & cheater_id)
{
	...
	if (!tmp_cheater)
	{
		LogInfo("! ERROR: SV: client [%u] not found ...", cheater_id.value());
		return;
	}
	if (m_ft_server->is_receiving_active(cheater_id))
	{
		LogInfo("! Receiving from client [%u] already active, please try later", cheater_id.value());
		return;
	}
													...
													u32 total)
{
	...
	case file_transfer::receiving_data:
		{
			LogInfo("* downloaded %d from %d bytes of screenshot from client [%d]", downloaded, total, m_chearer_id);
			if (m_first_receive)
			{
	...
	case file_transfer::receiving_aborted_by_peer:
		{
			LogInfo("* download screenshot aborted by peer [%u]", m_chearer_id);
			LPCSTR error_msg;
			char bufforint[16];
													...
													u32 total)
{
	...
	case file_transfer::receiving_data:
		{
			LogInfo("* downloaded %d from %d bytes of config from client [%d]", downloaded, total, m_chearer_id);
			if (m_first_receive)
			{
	...
	case file_transfer::receiving_aborted_by_peer:
		{
			LogInfo("* download config aborted by peer [%u]", m_chearer_id);
			LPCSTR error_msg;
			char bufforint[16];
												  ...
												  u32 total)
{
	...
	case file_transfer::sending_data:
		{
			LogInfo("* uploaded %d from %d bytes to client [%d]", uploaded, total, m_admin_id);
		}break;
	...
	case file_transfer::sending_rejected_by_peer:
		{
			LogInfo("* upload file terminated by peer [%d]", m_admin_id);
		}break;
	case file_transfer::sending_complete:
		{
			LogInfo("* upload file to admin [%d] complete !", m_admin_id);
		}break;
	};
}