...
//		LogInfo("[%6d][%s] actuality is false 2",Device.dwFrame,*object().cName());
	...
	switch (m_path_state) {
		...
		case ePathStatePathVerification : {
			if (!patrol().actual()) {
//				LogInfo("[%6d][%s] actuality is false 3",Device.dwFrame,*object().cName());
				m_path_state	= ePathStateSelectPatrolPoint;
			}
			else
...
//							LogInfo("[%6d][%s] detail path is completed",Device.dwFrame,*object().cName());
//							LogInfo(
...
//							);
		...
		case ePathStatePathCompleted : {
			if (!patrol().actual()) {
//				LogInfo("[%6d][%s] actuality is false 4",Device.dwFrame,*object().cName());
				m_path_state	= ePathStateSelectPatrolPoint;
			}
			break;
		}
