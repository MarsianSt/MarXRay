...
void profile_store::shedule_Update(u32 dt)
{
	VERIFY(m_fullgs_obj);
#ifdef DEBUG
	LogInfo("--- GameSpy core (SAKE) thinking ...");
#endif
											 ...
											 store_operation_cb complete_cb)
{
	if (m_complete_cb)
	{
		LogInfo("! ERROR: loading already in progress.");
		if (complete_cb)
		{
...
void profile_store::loaded_fields(bool const result, char const * err_descr)
{
	if (!m_complete_cb)
	{
		LogInfo("WARNING: loading awards terminated by user");
		VERIFY(!m_progress_indicator);
		Engine.Sheduler.Unregister(this);
		return;
	}
...
void __stdcall	profile_store::onlylog_operation(bool const result, char const * descr)
{
	if (!result)
	{
		LogInfo("! Profile store ERROR: %s", descr ? descr : "unknown");
		return;
	}
	LogInfo("* Profile store: %s", descr ? descr : "");
}
...
void __stdcall profile_store::onlylog_completion(bool const result, char const * err_descr)
{
	if (!result)
	{
		LogInfo("! Profile loading ERROR: %s", err_descr ? err_descr : "unknown");
		return;
	}
	LogInfo("* Profile loading successfully complete");
}
