...
void CWeaponMagazined::OnAnimationEnd(u32 state) 
{
	...
	case eReload:
	{
		CheckMagazine(); // ќсновано на механизме из Lost Alpha: New Project
						 // јвторы: rafa & Kondr48
	...
	case kWPN_RELOAD:
		{
			if (Actor()->mstate_real & (mcSprint) && !GameConstants::GetReloadIfSprint())
				...
				if (iAmmoElapsed < iMagazineSize || IsMisfire())
				{
					...
					if (!Det)
						Reload(); // Rietmon: ≈сли в слоте нету детектора, то он не может быть активен
...
//текуща€ дисперси€ (в радианах) оружи€ с учетом используемого патрона и недисперсионных пуль
