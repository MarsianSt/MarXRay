...
CSE_Abstract *CALifeSimulator__spawn_item2		(CALifeSimulator * self_, LPCSTR section, const Fvector &position, u32 level_vertex_id, GameGraph::_GRAPH_ID game_vertex_id, ALife::_OBJECT_ID id_parent)
{
	...
	if (!object) {
		LogInfo("! invalid parent id [%d] specified",id_parent);
		return							(0);
	}
...
//void disable_info_portion						(const CALifeSimulator *self_, const ALife::_OBJECT_ID &id)
//{
//	THROW								(self_);
//}

//void give_info_portion							(const CALifeSimulator *self_, const ALife::_OBJECT_ID &id)
//{
//	THROW								(self_);
//}
...
void iterate_objects(const CALifeSimulator* self_, luabind::functor<bool> functor)
{
    THROW(self_);
    for (const auto& it : self_->objects().objects())
    {
        CSE_ALifeDynamicObject* obj = it.second;
        if (functor(obj))
            return;
