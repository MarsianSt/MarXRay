...
#	endif
	...
	while (!object->children.empty()) {
		...
		R_ASSERT2			(child, make_string("child registered but not found [%d] for parent id=%d",object->children.back(), object->ID));
//		LogInfo("SLS-CLEAR : REJECT  [%s][%s] FROM [%s][%s]",child->name(),child->name_replace(),object->name(),object->name_replace());
...
//	LogInfo("SLS-CLEAR : DESTROY [%s][%s]",object->name(),object->name_replace());
...
{
#if 0
	LogInfo("SLS-CLEAR : %d objects");
	xrS_entities::const_iterator		I = entities.begin();
	xrS_entities::const_iterator		E = entities.end();
	for ( ; I != E; ++I)
		LogInfo("entity to destroy : [%d][%s][%s]",(*I).second->ID,(*I).second->name(),(*I).second->name_replace());
#endif
	...
	while (!entities.empty()) {
		...
		if (!found)		//R_ASSERT(found);
		{
			...
			{
				if (I->second)
					LogInfo("! ERROR: can't destroy object [%d][%s] with parent [%d]",
						I->second->ID, I->second->s_name.size() ? I->second->s_name.c_str() : "unknown",
						I->second->ID_Parent
					);
				else
					LogInfo("! ERROR: can't destroy entity [%d][?] with parent[?]", I->first);

			}
			LogInfo("! ERROR: FATAL: can't delete all entities !");
			entities.clear();
		}
