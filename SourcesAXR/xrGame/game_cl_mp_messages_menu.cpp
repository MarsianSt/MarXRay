		...
		for (u32 s=1; s<=16; s++)
		{
			...
			for (int t=1; t<=GetTeamCount(); t++)
			{
				...
				if (FS.exist(fn,"$game_sounds$",FileName_Voice,".ogg") && FS.exist(fn,"$game_sounds$",FileName_Radio,".ogg"))
				{
					...
					pMsgSound->mSound_Voice.create(FileName_Voice,st_Effect,sg_SourceType);	//LogInfo("-- %s Loaded", FileName_Voice);
					pMsgSound->mSound_Radio.create(FileName_Radio,st_Effect,sg_SourceType);	//LogInfo("-- %s Loaded", FileName_Radio);
				}
				else
				{
					R_ASSERT(0);
...
//	LogInfo("Variant %d from %d", VariantID, pMMessage->aVariants.size());
	//-------------------------------------------------------------------
	NET_Packet P;
