...
void	animation_movement_controller::NewBlend	( CBlend* B, const Fmatrix &new_matrix, bool local_animation )
{
	...
	if( ControlBlend( )->playing )
		LogInfo( " ! obj movement anim not yet ended anim: %s anim set: %s \n and already another started anim: %s anim set: %s", 	
			new_anim_name,new_anim_set,old_anim_name,old_anim_set
			);
	if( !ControlBlend( )->stop_at_end )
		LogInfo( " ! obj movement anim  : %s anim set: %s  is not stop-at-end but fallowed in chain by another obj movement anim: %s anim set: %s", 	
			old_anim_name,old_anim_set,new_anim_name,new_anim_set
			);
	if( !B->stop_at_end )
		LogInfo( " ! obj movement anim  : %s anim set: %s  is not stop-at-end but fallowing after another obj movement anim: %s anim set: %s", 	
			new_anim_name,new_anim_set,old_anim_name,old_anim_set
			);
#endif
	//	VERIFY(  );
