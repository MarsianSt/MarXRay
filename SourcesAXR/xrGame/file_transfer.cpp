...
void server_site::update_transfer()
{
			...
			tie = m_transfers.end(); ti != tie; ++ti)
	{
		...
		if (!tmp_client)
		{
			LogInfo("! ERROR: SV: client [%u] not found for transfering file", ti->first);
										...
										sending_state_callback_t & tstate_callback)
{
	...
	{
		LogInfo("! ERROR: SV: transfering file to client [%d] already active.", to_client);
		return;
	}
	...
	if (!ftnode->opened())
	{
		LogInfo("! ERROR: SV: failed to open file [%s]", file_name.c_str());
...
void server_site::stop_transfer_file(dst_src_pair_t const & tkey)
{
	...
	if (temp_iter == m_transfers.end())
	{
		LogInfo("! ERROR: SV: no file transfer for client [%d] found from client [%d].", 
										...
										receiving_state_callback_t & rstate_callback)
{
	...
	if (temp_iter != m_receivers.end())
	{
		LogInfo("! ERROR: SV: file already receiving from client [%d]", from_client);
		return NULL;
	}
	...
	{
		LogInfo("! ERROR: SV: failed to create file [%s]", file_name.c_str());
...
void server_site::stop_receive_file(ClientID const & from_client)
{
	...
	if (temp_iter == m_receivers.end())
	{
		LogInfo("! ERROR: SV: no file receiving from client [%u] found", from_client);
		return;
	}
	if (!temp_iter->second->is_complete())
	{
	...
	case abort_receive:
		{
			...
			{
				LogInfo("! WARNING: CL: server sent unknown abort receive message");
			}
		}break;
	case receive_rejected:
		{
			...
			{
				LogInfo("! WARNING: CL: server sent unknown receive reject message"); 
			}
		}break;
	};
}
											...
											u32 size_to_allocate)
{
	...
	if (!size || !data)
	{
		LogInfo("! ERROR: CL: no data to transfer ...");
		return;
	}
										...
										receiving_state_callback_t & rstate_callback)
{
	...
	{
		LogInfo("! ERROR: CL: file already receiving from client [%d]", from_client);
		return NULL;
	}
	filereceiver_node* frnode = xr_new<filereceiver_node>(file_name, rstate_callback);
	m_receivers.insert(std::make_pair(from_client, frnode));
	if (!frnode->get_writer())
	{
		LogInfo("! ERROR: CL: failed to create file [%s]", file_name.c_str());
		stop_receive_file(from_client);
		return NULL;
	}
	return frnode;
...
void client_site::stop_receive_file(ClientID const & from_client)
{
	...
	if (temp_iter == m_receivers.end())
	{
		LogInfo("! ERROR: CL: no file receiving from client [%u] found", from_client);
		return;
	}
	if (!temp_iter->second->is_complete())
	{
