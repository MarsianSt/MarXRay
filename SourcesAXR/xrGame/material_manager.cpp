...
void CMaterialManager::reinit		()
{
	...
	CEntityAlive			*entity_alive = smart_cast<CEntityAlive*>(m_object);
	if (entity_alive) {
		//VERIFY( entity_alive->character_physics_support()->get_movement()->CharacterExist() );
		entity_alive->character_physics_support()->get_movement()->SetPLastMaterialIDX	(&m_last_material_idx);
...
//		}
		...
		entity_alive->character_physics_support()->get_movement()->SetMaterial		(m_my_material_idx);
#ifdef	DEBUG
		if( debug_character_material_load )
		{
			VERIFY( GAMEMTL_NONE_IDX != m_my_material_idx );
