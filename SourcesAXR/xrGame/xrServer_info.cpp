...
void xrServer::LoadServerInfo()
{
	...
	if (!m_server_logo)
	{
		LogInfo("! ERROR: failed to open server logo file %s", SERVER_LOGO_FN);
		return;
	}
	m_server_rules = FS.r_open("$app_data_root$", SERVER_RULES_FN);
	if (!m_server_rules)
	{
		LogInfo("! ERROR: failed to open server rules file %s", SERVER_RULES_FN);
	...
	u32 uploaded, u32 total)
{
	...
	case file_transfer::sending_data:
		{
#ifdef DEBUG
			LogInfo("* uploaded %d from %d bytes of server logo to client [%d]", uploaded, total, m_to_client.value());
#endif
			return;
		}break;
	...
	case file_transfer::sending_rejected_by_peer:
		{
			LogInfo("* upload server logo terminated by peer [%d]", m_to_client.value());
		}break;
	case file_transfer::sending_complete:
		{
			LogInfo("* upload server info to client [%d] complete !", m_to_client.value());
		}break;
	};
	m_state = eUploadNotActive;
	execute_complete_cb();
