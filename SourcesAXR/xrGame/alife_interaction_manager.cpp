		...
		switch (manager->m_tpaCombatObjects[l_iGroupIndex]->tfGetActionType(manager->m_tpaCombatObjects[l_iGroupIndex ^ 1],l_iGroupIndex,l_bMutualDetection)) {
			case eMeetActionTypeAttack : {
				...
				if (psAI_Flags.test(aiALife)) {
					LogInfo("[LSS] %s started combat versus %s",manager->m_tpaCombatObjects[l_iGroupIndex]->base()->name_replace(),manager->m_tpaCombatObjects[l_iGroupIndex ^ 1]->base()->name_replace());
				}
#endif
...
#ifdef DEBUG
						if (psAI_Flags.test(aiALife)) {
							LogInfo("[LSS] %s choosed to attack %s",manager->m_tpaCombatObjects[l_iGroupIndex]->base()->name_replace(),manager->m_tpaCombatObjects[l_iGroupIndex ^ 1]->base()->name_replace());
						}
#endif
...
#ifdef DEBUG
						if (psAI_Flags.test(aiALife)) {
							LogInfo("[LSS] %s choosed to retreat from %s",manager->m_tpaCombatObjects[l_iGroupIndex]->base()->name_replace(),manager->m_tpaCombatObjects[l_iGroupIndex ^ 1]->base()->name_replace());
						}
#endif
						if (manager->bfCheckIfRetreated(l_iGroupIndex)) {
#ifdef DEBUG
							if (psAI_Flags.test(aiALife)) {
								LogInfo("[LSS] %s did retreat from %s",manager->m_tpaCombatObjects[l_iGroupIndex]->base()->name_replace(),manager->m_tpaCombatObjects[l_iGroupIndex ^ 1]->base()->name_replace());
							}
#endif
...
#ifdef DEBUG
						if (psAI_Flags.test(aiALife)) {
							LogInfo("[LSS] %s didn't retreat from %s",manager->m_tpaCombatObjects[l_iGroupIndex]->base()->name_replace(),manager->m_tpaCombatObjects[l_iGroupIndex ^ 1]->base()->name_replace());
						}
#endif
					}
...
#ifdef DEBUG
						if (psAI_Flags.test(aiALife)) {
							LogInfo("[LSS] %s is dead",manager->m_tpaCombatObjects[l_iGroupIndex]->base()->name_replace());
						}
#endif
				...
				if (psAI_Flags.test(aiALife)) {
					if (eCombatResultRetreat12 == l_tCombatResult)
						LogInfo("[LSS] both combat groups decided not to continue combat");
				}
#endif
...
#ifdef DEBUG
				if (psAI_Flags.test(aiALife)) {
					LogInfo("[LSS] %s interacted with %s",manager->m_tpaCombatObjects[l_iGroupIndex]->base()->name_replace(),manager->m_tpaCombatObjects[l_iGroupIndex ^ 1]->base()->name_replace());
				}
#endif
...
#ifdef DEBUG
				if (psAI_Flags.test(aiALife)) {
					LogInfo("[LSS] %s refused from combat",manager->m_tpaCombatObjects[l_iGroupIndex]->base()->name_replace());
				}
#endif
				return;
			}
