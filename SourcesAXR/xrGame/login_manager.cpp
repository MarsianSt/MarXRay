									 ...
									 login_operation_cb logincb)
{
	...
	{
		LogInfo("! WARNING: first you need to log out...");
...
void login_manager::login_offline(char const * nick, login_operation_cb logincb)
{
	...
	{
		LogInfo("! WARNING: login in process...");
		return;
	}
	if (!logincb)
	{
										...
										login_operation_cb logincb)
{
	...
	if (!m_current_profile)
	{
		LogInfo("! WARNING: first you need to log in...");
											 ...
											 char const * description)
{
	if (!res_profile)
	{
		LogInfo("! GameSpy login ERROR: %s", 
			description ? description : "unknown");
		return;
	}
	LogInfo("* GameSpy login operation success ! Hello %s !", res_profile->m_unique_nick.c_str());
}
									 ...
									 void * param)
{
	...
	{
		LogInfo("! ERROR: failed to get login ticket");
		tmp_ticket_dest[0] = 0;
	}
