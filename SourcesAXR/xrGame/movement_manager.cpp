...
//				LogInfo("[%6d][%s] actuality is false",Device.dwFrame,*object().cName());
...
{
//	LogInfo("[%6d][%s][on_restrictions_change]",Device.dwTimeGlobal,*object().cName());
	m_path_actuality				= false;
	level_path_builder().remove		();
	detail_path_builder().remove	();
...
//	static int i=0;
//	LogInfo("[%6d][%6d][%4d][%f] build_level_path",Device.dwTimeGlobal,Device.dwFrame,++i,timer.GetElapsed_sec()*1000.f);
}
