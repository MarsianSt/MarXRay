...
void CSightManager::Exec_Look		(float time_delta)
{
	...
	if ( object().cName() == "level_prefix_stalker" ) {
		LogInfo("[%6d][%s] BEFORE BODY [%f] -> [%f]",Device.dwTimeGlobal, object().cName().c_str(), object().get_movement().m_body.current.yaw,object().get_movement().m_body.target.yaw);
		LogInfo("[%6d][%s] BEFORE HEAD [%f] -> [%f]",Device.dwTimeGlobal, object().cName().c_str(), object().get_movement().m_head.current.yaw,object().get_movement().m_head.target.yaw);
	}
...
#ifdef DEBUG
	if ( g_ai_dbg_sight )
		LogInfo( "%6d [%s] before body[%f]->[%f], head[%f]->[%f]", Device.dwTimeGlobal, object().cName().c_str(), body.current.yaw, body.target.yaw, head.current.yaw, head.target.yaw );
#endif // #ifdef DEBUG
	vfValidateAngleDependency		(body.current.yaw,body.target.yaw,head.current.yaw);
#ifdef DEBUG
	if ( g_ai_dbg_sight )
		LogInfo( "%6d [%s] after  body[%f]->[%f], head[%f]->[%f]", Device.dwTimeGlobal, object().cName().c_str(), body.current.yaw, body.target.yaw, head.current.yaw, head.target.yaw );
#endif // #ifdef DEBUG
...
#ifdef DEBUG
	if ( g_ai_dbg_sight )
		LogInfo( "%6d [%s] after2 body[%f]->[%f], head[%f]->[%f]", Device.dwTimeGlobal, object().cName().c_str(), body.current.yaw, body.target.yaw, head.current.yaw, head.target.yaw );
#endif // #ifdef DEBUG
	...
	if ( object().cName() == "level_prefix_stalker" ) {
		LogInfo("[%6d][%s] AFTER  BODY [%f] -> [%f]",			Device.dwTimeGlobal, object().cName().c_str(),object().get_movement().m_body.current.yaw,object().get_movement().m_body.target.yaw);
		LogInfo("[%6d][%s] AFTER  HEAD [%f][%f] -> [%f][%f]",	Device.dwTimeGlobal, object().cName().c_str(), object().get_movement().m_head.current.yaw,object().get_movement().m_head.current.pitch,object().get_movement().m_head.target.yaw,object().get_movement().m_head.target.pitch);
	}
#endif // #ifdef SIGHT_DEBUG
...
#ifdef DEBUG
	if ( g_ai_dbg_sight )
		LogInfo( "%6d [%s] after3 body[%f]->[%f], head[%f]->[%f]", Device.dwTimeGlobal, object().cName().c_str(), body.current.yaw, body.target.yaw, head.current.yaw, head.target.yaw );
#endif // #ifdef DEBUG
...
//			LogInfo("%6d started turning in place",Device.dwTimeGlobal);
...
//		object().get_movement().m_body.target.yaw	= object().get_movement().m_head.current.yaw;
		...
		m_turning_in_place	= false;
//		LogInfo("%6d stopped turning in place",Device.dwTimeGlobal);
		object().get_movement().m_body.target.yaw	= object().get_movement().m_body.current.yaw;
	}
...
//	if ( object().cName() == "level_prefix_stalker" ) {
//		LogInfo("[%6d][%6d] [%f] + [%f] = [%f] ([%f])",  Device.dwFrame, Device.dwTimeGlobal, m_current.m_head.m_factor,		s_factor_lerp_speed*time_delta,		lerp ( m_current.m_head.m_factor,		factors.x, s_factor_lerp_speed*time_delta ), factors.x );
//		LogInfo("[%6d][%6d] [%f] + [%f] = [%f] ([%f])",  Device.dwFrame, Device.dwTimeGlobal, m_current.m_shoulder.m_factor,	s_factor_lerp_speed*time_delta,		lerp ( m_current.m_shoulder.m_factor,	factors.y, s_factor_lerp_speed*time_delta ), factors.y );
//		LogInfo("[%6d][%6d] [%f] + [%f] = [%f] ([%f])",  Device.dwFrame, Device.dwTimeGlobal, m_current.m_spine.m_factor,	s_factor_lerp_speed*time_delta,		lerp ( m_current.m_spine.m_factor,		factors.z, s_factor_lerp_speed*time_delta ), factors.z );
//	}
	...
	switch (m_aiming_type) {
		...
		case aiming_head : {
			...
			if (!forward_blend_callbacks && !backward_blend_callbacks) {
				if (!fis_zero(time_delta)) {
#ifdef DEBUG
					LogInfo( "!animation movement controller wasn't created" );
#endif // #ifdef DEBUG
...
void CSightManager::enable							(bool const value)
{
	...
	m_enabled			= value;
//	LogInfo("[%d][%s] sight_enabled[%c]", Device.dwTimeGlobal, object().cName().c_str(), value ? '+' : '-');
