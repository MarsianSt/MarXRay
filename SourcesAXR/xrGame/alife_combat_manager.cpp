...
void print_time(LPCSTR S, _TIME_ID tTimeID)
{
	...
	Years = u32(tTimeID) + 1;
	LogInfo("%s year %d month %d week %d day %d time %d:%d:%d.%d",S,Years,Months,Week,Days,Hours,Minutes,Seconds,Milliseconds);
}
...
bool CALifeCombatManager::bfCheckForInteraction(CSE_ALifeSchedulable *tpALifeSchedulable1, CSE_ALifeSchedulable *tpALifeSchedulable2, int &iCombatGroupIndex, bool &bMutualDetection)
{
	...
	if (psAI_Flags.test(aiALife)) {
		...
		LogInfo("[LSS] %s met %s on the graph point %d (level %s[%d][%d][%d][%d], [%f][%f][%f])",
...
#ifdef DEBUG
		if (psAI_Flags.test(aiALife)) {
			LogInfo("[LSS] %s detected %s",tpALifeSchedulable1->base()->name_replace(),tpALifeSchedulable2->base()->name_replace());
		}
#endif
	}
	else {
#ifdef DEBUG
		if (psAI_Flags.test(aiALife)) {
			LogInfo("[LSS] %s didn't detect %s",tpALifeSchedulable1->base()->name_replace(),tpALifeSchedulable2->base()->name_replace());
		}
#endif
	}
...
#ifdef DEBUG
		if (psAI_Flags.test(aiALife)) {
			LogInfo("[LSS] %s detected %s",tpALifeSchedulable2->base()->name_replace(),tpALifeSchedulable1->base()->name_replace());
		}
#endif
...
#ifdef DEBUG
		if (psAI_Flags.test(aiALife)) {
			LogInfo("[LSS] %s didn't detect %s",tpALifeSchedulable2->base()->name_replace(),tpALifeSchedulable1->base()->name_replace());
		}
#endif
	}
...
#ifdef DEBUG
		if (psAI_Flags.test(aiALife)) {
			LogInfo("[LSS] There is no interaction");
		}
#endif
		return						(false);
	}
...
#ifdef DEBUG
		if (psAI_Flags.test(aiALife)) {
			LogInfo("[LSS] %s attacks with %s(%d ammo) %d times in a row",(*I)->base()->name_replace(),(*I)->m_tpCurrentBestWeapon ? (*I)->m_tpCurrentBestWeapon->name_replace() : "its natural weapon",(*I)->m_tpCurrentBestWeapon ? (*I)->m_tpCurrentBestWeapon->m_dwAmmoAvailable : 0,iFloor(ai().ef_storage().m_pfWeaponAttackTimes->ffGetValue() + .5f));
		}
#endif
...
#ifdef DEBUG
				if (psAI_Flags.test(aiALife)) {
					LogInfo("[LSS] %s missed",(*I)->base()->name_replace());
				}
#endif
			}
...
#ifdef DEBUG
		if (psAI_Flags.test(aiALife)) {
			LogInfo("[LSS] There is nothing to take");
		}
#endif
		return;
	}
...
#ifdef DEBUG
		if (psAI_Flags.test(aiALife)) {
			LogInfo("[LSS] Starting taking items [%s][%f]",m_tpaCombatObjects[l_iGroupIndex]->base()->name_replace(),smart_cast<CSE_ALifeMonsterAbstract*>(m_tpaCombatObjects[l_iGroupIndex])->fHealth);
		}
#endif
