...
IC	bool CGameVertexPathManager::is_accessible	(const _index_type &vertex_id) const
{
	...
	typedef _Parameters::VERTEX_TYPES::const_iterator	const_iterator_t;
#ifdef DEBUG
	if (m_evaluator->m_vertex_types->empty()) {
		LogInfo("! warning : empty vertex types");
	}
#endif
	const_iterator_t		I = m_evaluator->m_vertex_types->begin();
	const_iterator_t		E = m_evaluator->m_vertex_types->end();
	for ( ; I != E; ++I)
