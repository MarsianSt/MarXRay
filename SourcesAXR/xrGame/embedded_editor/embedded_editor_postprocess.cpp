...
void ApplyEffect(const SPPEffect& eff)
{
    ...
    if (eff.section.size())
    {
        if (eff.m_bIsShootEffector)
        {
            ...
            if (Wpn)
            {
                float disp = ((Wpn->GetFireDispersion(Wpn->m_fCurrentCartirdgeDisp, Wpn->IsZoomed()) * 150.f) * (Wpn->IsZoomed() ? effector_intensity_aim : Actor()->is_actor_crouch() ? effector_intensity_crouch : effector_intensity));
...
void ShowPostprocessEditor(bool& show)
{
    ...
    if (m_current_effect >= 0 && m_current_effect < (int)m_effects.size())
    {
        ...
        ImGui::DragFloat("Shoot Factor", &eff.m_fShootEffectorFactor, 0.001f, 0.0f, 10.f);
        ImGui::DragFloat("Shoot Factor Aim", &eff.m_fShootEffectorFactorAim, 0.001f, 0.0f, 10.f);
        ImGui::DragFloat("Shoot Factor Crouch", &eff.m_fShootEffectorFactorCrouch, 0.001f, 0.0f, 10.f);
        ...
        for (size_t i = 0; i < eff.pp_effects.size(); i++)
        {
            ...
            if (editEffect(buf, eff.pp_effects[i], true))
            {
            }
        ...
        for (size_t i = 0; i < eff.cam_effects.size(); i++)
        {
            ...
            if (editEffect(buf, eff.cam_effects[i], false))
            {
            }
