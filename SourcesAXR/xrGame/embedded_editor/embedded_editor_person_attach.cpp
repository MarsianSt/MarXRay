...
void SaveToFile(CAttachmentOwner* owner)
{
	...
	if (CWeapon* Wpn1 = smart_cast<CWeapon*>(Actor()->inventory().ItemFromSlot(INV_SLOT_2)))
	{
		// In hands
	...
	if (CWeapon* Wpn2 = smart_cast<CWeapon*>(Actor()->inventory().ItemFromSlot(INV_SLOT_3)))
	{
		// In hands
	...
	if (CWeapon* Pistol = smart_cast<CWeapon*>(Actor()->inventory().ItemFromSlot(PISTOL_SLOT)))
	{
		// In hands
		pAttachesCfg.w_string(Pistol->m_section_id.c_str(),
			"position",
			make_string("%f,%f,%f", Pistol->GetHandsOffsetPos().x, Pistol->GetHandsOffsetPos().y, Pistol->GetHandsOffsetPos().z)
			.c_str());

		pAttachesCfg.w_string(Pistol->m_section_id.c_str(),
			"orientation",
			make_string("%f,%f,%f", Pistol->GetHandsOffsetRot().x, Pistol->GetHandsOffsetRot().y, Pistol->GetHandsOffsetRot().z)
			.c_str());

		const char* wpn_sect_name = Pistol->m_section_id.c_str();

		string_path fname_main;
		FS.update_path(fname_main, "$app_data_root$", make_string("NPC_AttachesEditor\\%s.ltx", wpn_sect_name).c_str());

		if (Pistol->LaserAttachable())
		{
			pAttachesCfg.w_string(wpn_sect_name,
				"laserdot_world_attach_offset",
				make_string("%f,%f,%f", Pistol->laserdot_attach_offset.x, Pistol->laserdot_attach_offset.y, Pistol->laserdot_attach_offset.z)
				.c_str());
		}

		if (Pistol->TacticalTorchAttachable())
		{
			pAttachesCfg.w_string(wpn_sect_name,
				"torch_world_attach_offset",
				make_string("%f,%f,%f", Pistol->flashlight_attach_offset.x, Pistol->flashlight_attach_offset.y, Pistol->flashlight_attach_offset.z)
				.c_str());

			pAttachesCfg.w_string(wpn_sect_name,
				"torch_omni_world_attach_offset",
				make_string("%f,%f,%f", Pistol->flashlight_omni_attach_offset.x, Pistol->flashlight_omni_attach_offset.y, Pistol->flashlight_omni_attach_offset.z)
				.c_str());
		}

		if (Pistol->m_weapon_attaches.size())
			SaveAttachesCfg(Pistol->cNameSect().c_str(), Pistol);
	}

	if (CWeapon* binocular = smart_cast<CWeapon*>(Actor()->inventory().ItemFromSlot(BINOCULAR_SLOT)))
	{
		// In hands
...
void ShowPersonAttachEditor(bool& show)
{
	...
	if (Actor() && Actor()->g_Alive())
	{
		...
		if (CAttachmentOwner* owner = smart_cast<CAttachmentOwner*>(Actor()))
		{
			...
			if (CWeapon* Pistol = smart_cast<CWeapon*>(Actor()->inventory().ItemFromSlot(PISTOL_SLOT)))
				EditWeaponParameters(Pistol, drag_intensity);
