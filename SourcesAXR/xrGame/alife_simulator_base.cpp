...
//	LogInfo("LSS : SPAWN : [%s],[%s], level %s",*dynamic_object->s_name,dynamic_object->name_replace(),*ai().game_graph().header().level(ai().game_graph().vertex(dynamic_object->m_tGraphID)->level_id()).name());
...
#ifdef DEBUG
//	LogInfo("Creating object from client spawn [%d][%d][%s][%s]",dynamic_object->ID,dynamic_object->ID_Parent,dynamic_object->name(),dynamic_object->name_replace());
#endif
...
void CALifeSimulatorBase::release	(CSE_Abstract *abstract, bool alife_query)
{
	...
	if (psAI_Flags.test(aiALife)) {
		LogInfo("[LSS] Releasing object [%s][%s][%d][%x]",abstract->name_replace(),*abstract->s_name,abstract->ID,smart_cast<void*>(abstract));
	}
#endif
...
#ifdef DEBUG
	if (psAI_Flags.test(aiALife)) {
		LogInfo("[LSS] Generated death position %s[%f][%f][%f] -> [%f][%f][%f] : [%d]",tpALifeCreatureAbstract->name_replace(),VPUSH(tpALifeCreatureAbstract->o_Position),VPUSH((*i).level_point()),(*i).level_vertex_id());
	}
#endif
