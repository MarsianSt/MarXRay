...
void CCC_CreateGameSpyAccount::Execute(LPCSTR args)
{
	...
	sscanf_s(args, "%s %s %s %s",
		tmp_nick, static_cast<unsigned>(sizeof(tmp_nick)),
		tmp_unick, static_cast<unsigned>(sizeof(tmp_unick)),
		tmp_email, static_cast<unsigned>(sizeof(tmp_email)),
		tmp_password, static_cast<unsigned>(sizeof(tmp_password)));
...
void CCC_GameSpyPrintProfile::Execute(LPCSTR args)
{
	...
	if (tmp_profile)
	{
		LogInfo("- Current profile:");
		LogInfo("- ProfileID  : %u", tmp_profile->m_profile_id);
		LogInfo("- UniqueNick : %s", tmp_profile->m_unique_nick.c_str());

		gamespy_profile::profile_store* tmp_store = MainMenu()->GetProfileStore();
		if (!tmp_store)
		{
			LogInfo("! No profile store available");
			return;
		}
		
		LogInfo("- Player awards:");
	...
	{
		LogInfo("- No profile. You are not loged in.");
	}
}
...
void CCC_GameSpySuggestUNicks::Execute(LPCSTR args)
{
	...
	VERIFY(MainMenu() && MainMenu()->GetGS());
	string256	tmp_unick;
	sscanf_s(args, "%s", tmp_unick, static_cast<unsigned>(sizeof(tmp_unick)));
...
void CCC_GameSpyProfile::Execute(LPCSTR args)
{
	...
	sscanf_s(args, "%s", tmp_command, static_cast<unsigned>(sizeof(tmp_command)));
