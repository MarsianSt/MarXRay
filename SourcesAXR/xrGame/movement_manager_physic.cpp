...
void dump_collision_hit(CPHMovementControl *movement_control)
{
	...
	LogInfo( "ai unit: %s hited by collision; power: %f, spawn frame %d, current frame %d ", obj->cName().c_str(), movement_control->gcontact_HealthLost, obj->spawn_time(), Device.dwFrame ); 
...
//		LogInfo("[%6d][%s] no move, curr_tp=%d",Device.dwFrame,*object().cName(),detail().m_current_travel_point);
		return;
	}
...
#pragma todo("Dima to ? : is this correct?")
		...
		m_speed			= 0.f;
		//LogInfo("[%6d][%s] strange exit, curr_tp=%d",Device.dwFrame,*object().cName(),detail().m_current_travel_point);
		return;
	}
//	LogInfo("[%6d][%s] curr_tp=%d",Device.dwFrame,*object().cName(),detail().m_current_travel_point);
