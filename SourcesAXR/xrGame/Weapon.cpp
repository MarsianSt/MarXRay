...
CWeapon::CWeapon()
{
	...
	m_bOrigCamEffForAdvanced = false;
	...
	m_WeaponBonesController = nullptr;
...
CWeapon::~CWeapon		()
{
	...
	if (m_WeaponBonesController)
		xr_delete			(m_WeaponBonesController);
...
//	m_zoom_params.m_bZoomDofEnabled	= !def_dof.similar(m_zoom_params.m_ZoomDof);
	...
	m_bOrigCamEffForAdvanced = READ_IF_EXISTS(pSettings, r_bool, section, "orig_cam_effects_for_advanced", false);
	...
	if (pSettings->line_exist(section, "controlled_bones") && !m_WeaponBonesController)
		m_WeaponBonesController = xr_new<CWeaponBonesController>();
...
void CWeapon::on_a_hud_attach()
{
	...
	if (m_WeaponBonesController && GetHUDmode())
		m_WeaponBonesController->SetCallbacks(HudItemData()->m_model);
}
...
void CWeapon::on_b_hud_detach()
{
	...
	if (m_WeaponBonesController && GetHUDmode())
		m_WeaponBonesController->ResetCallbacks(HudItemData()->m_model);
}
...
void CWeapon::UpdateCL		()
{
	...
	if (m_WeaponBonesController && GetHUDmode())
		m_WeaponBonesController->Update();
...
void CWeapon::update_visual_bullet_textures(const bool forced)
{
	...
	const u32 id = m_set_next_ammoType_on_reload != undefined_ammo_type ? m_set_next_ammoType_on_reload : m_ammoType;
