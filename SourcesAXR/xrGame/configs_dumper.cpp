...
void configs_dumper::write_configs()
{
	...
	for (active_objects_t::size_type idx = 0; idx < aobjs_count; ++idx)
	{
		xr_sprintf				(tmp_strbuff, "%d", idx + 1);
		m_active_params.dump	(active_objects[idx], tmp_strbuff, active_params_dumper);
	}
...
void configs_dumper::dump_config(complete_callback_t complete_cb)
{
	...
	{
#ifdef DEBUG
		LogInfo("! ERROR: CL: dump making already in progress...");
#endif
		return;
	}
