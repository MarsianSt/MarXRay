...
//		LogInfo("%6d : Stalker %s : grenade reaction",Device.dwTimeGlobal,*m_object->cName());
		...
		if (initiator) {
			if (is_relation_enemy(initiator))
				...
				if (missile->Position().distance_to(Position()) < FRIENDLY_GRENADE_ALARM_DIST) {
					...
					get_sound().play	( StalkerSpace::eStalkerSoundFriendlyGrenadeAlarm, time + 1500, time + 1000 );
				}
		}
	}
...
void CAI_Stalker::react_on_member_death	()
{
	...
	if (agent_manager().get_member().group_behaviour()) {
		if (!reaction.m_member->g_Alive())
			get_sound().play		( StalkerSpace::eStalkerSoundTolls, 3000, 2000 );
		else
			get_sound().play		( StalkerSpace::eStalkerSoundWounded, 3000, 2000 );
	}
