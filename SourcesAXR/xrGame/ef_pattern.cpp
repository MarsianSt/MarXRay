...
void CPatternFunction::vfLoadEF(LPCSTR caFileName)
{
	...
	if (!FS.exist(caPath,"$game_ai$",caFileName)) {
		LogInfo("! Evaluation function : File not found \"%s\"",caPath);
		R_ASSERT	(false);
		return;
	}
	...
	if (EFC_VERSION != m_tEFHeader.dwBuilderVersion) {
		FS.r_close	(F);
		LogInfo("! Evaluation function (%s) : Not supported version of the Evaluation Function Contructor",caPath);
		R_ASSERT	(false);
		return;
	}
	...
	// LogInfo("* Evaluation function \"%s\" is successfully loaded",m_caName);
}
