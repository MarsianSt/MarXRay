...
#ifdef	DEBUG
		if( death_anim_debug )
			LogInfo("death anims: load: no setings in section %s for %s", section, type );
#endif
			return this;
		}
...
#ifdef	DEBUG
			if( death_anim_debug )
				LogInfo("death anims: load: loaded %s from section %s for %s", sdir_anim, section, type );
#endif
		}
	}
#ifdef	DEBUG
...
#ifdef	DEBUG
		if( death_anim_debug )
			LogInfo(" death anims: obj: %s no death motions loaded ", ea.cName().c_str() );
#endif
...
#ifdef	DEBUG
		if( death_anim_debug )
			LogInfo(" death anims: obj: %s no conditions evaluated  returns random ", ea.cName().c_str() );
#endif
	angle = 0;
	return rnd_anims.motion();
}
