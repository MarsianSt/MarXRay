...
#ifdef DEBUG
	LogInfo("* Loading ai space is successfully completed (%.3fs, %7.3f Mb)",timer.GetElapsed_sec(),float(Memory.mem_usage() - mem_usage)/1048576.0);
#endif
}
...
void CAI_Space::validate			(const u32 level_id) const
{
	...
	for (GameGraph::_GRAPH_ID i=0, n = game_graph().header().vertex_count(); i<n; ++i)
		if ((level_id == game_graph().vertex(i)->level_id()) && 
			...
			LogInfo("! Graph doesn't correspond to the cross table");
			R_ASSERT2		(false,"Graph doesn't correspond to the cross table");
		}

//	LogInfo("death graph point id : %d",cross_table().vertex(455236).game_vertex_id());
...
//		LogInfo("vertex [%d] has %d death points",i,game_graph().vertex(i)->death_point_count());
		for ( ; I != E; ++I) {
...
//	LogInfo("* Graph corresponds to the cross table");
}
#endif
