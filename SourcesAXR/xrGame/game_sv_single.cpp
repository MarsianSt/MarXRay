	...
	if (ai().get_alife()) {
		...
		if	(
			...
			if (psAI_Flags.test(aiALife)) {
				LogInfo("Cannot attach object [%s][%s][%d] to object [%s][%s][%d]",e_what->name_replace(),*e_what->s_name,e_what->ID,e_who->name_replace(),*e_who->s_name,e_who->ID);
			}
#endif
	}
	return TRUE;
	...
	if (ai().get_alife()) 
	{
		...
		if	(
			...
			if (!ai().alife().objects().object(e_what->ID,true)) {
				...
				if (psAI_Flags.test(aiALife)) {
					LogInfo("Cannot detach object [%s][%s][%d] from object [%s][%s][%d]", l_tpALifeInventoryItem->base()->name_replace(),
