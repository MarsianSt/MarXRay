...
void __stdcall reward_event_generator::AddRewardTask(u32 award_id)
{
	...
	if (!tmp_curr_prof)
	{
		LogInfo("! ERROR: can't reward player - not logged in");
		return;
	}
	if ((m_rewarded >= m_max_rewards) && (m_max_rewards != u32(-1)))
	{
		LogInfo("! You have been rewarded by award [%s], but maximum rewards per game reached... sorry :(",
	...
	{
		LogInfo("* An offline player has been rewarded by [%s] award",
...
void reward_event_generator::CommitBestResults()
{
	...
	if (!tmp_curr_prof)
	{
		LogInfo("! ERROR: can't submit best scores - not logged in");
		return;
	}
	if (!tmp_curr_prof->online())
	{
