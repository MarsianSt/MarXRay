...
void xrServer::Process_event_destroy	(NET_Packet& P, ClientID sender, u32 time, u16 ID, NET_Packet* pEPack)
{
	...
	if( dbg_net_Draw_Flags.test( dbg_destroy ) )
		LogInfo("sv destroy object %s [%d]", ent_name_safe(id_dest).c_str(), Device.dwFrame);
#endif
	...
	{
#ifndef MASTER_GOLD
		LogInfo("!SV:ge_destroy: [%d] not found on server",id_dest);
...
#ifdef MP_LOGGING
	LogInfo("--- SV: Process destroy: parent [%d] item [%d][%s]", 
