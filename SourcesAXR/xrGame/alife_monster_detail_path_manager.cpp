...
//	LogInfo("[%6d][%s][%f][%f][%f]",Device.dwTimeGlobal,object().name_replace(),VPUSH(m_destination.m_position));
}
...
void CALifeMonsterDetailPathManager::target					(const CALifeSmartTerrainTask *task)
{
//	LogInfo("[%6d][%s][%s]",Device.dwTimeGlobal,object().name_replace(),*task->patrol_path_name());
	target							(*task);
}
...
//		LogInfo("[detail::update][%6d][%s]",Device.dwTimeGlobal,object().name_replace());
...
#ifdef DEBUG
	if (failed) {
		LogInfo("! %s couldn't build game path from",object().get_object().name_replace());
		{
			...
			LogInfo("! game_graph_mask -> [ %d, %d, %d, %d]", vertex->vertex_type()[0], vertex->vertex_type()[1], vertex->vertex_type()[2], vertex->vertex_type()[3]);
		}
		...
		LogInfo("! List of available game_graph masks:");
...
void CALifeMonsterDetailPathManager::follow_path				(const ALife::_TIME_ID &time_delta)
{
	...
	for ( ; m_path.size() > 1;) {
		...
		m_walked_distance				= 0.f;
		m_path.pop_back					();
//		LogInfo("%6d %s changes graph point from %d to %d",Device.dwTimeGlobal,object().name_replace(),object().m_tGraphID,(GameGraph::_GRAPH_ID)m_path.back());
