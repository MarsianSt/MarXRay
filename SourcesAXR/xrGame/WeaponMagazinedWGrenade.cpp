...
void CWeaponMagazinedWGrenade::Load	(LPCSTR section)
{
	...
	m_ammoTypes2.clear	(); 
	LPCSTR				grenade_sect = pSettings->r_string(section, "grenade_class");

	if (grenade_sect && grenade_sect[0])
	{
		string128 _ammoItem;
		int count = _GetItemCount(grenade_sect);
...
void CWeaponMagazinedWGrenade::switch2_Unmis()
{
	...
	if (GrenadeLauncherAttachable() && IsGrenadeLauncherAttached())
	{
		...
		PlayHUDMotionIfExists({ anmUnmisName, "anm_reload_misfire_w_gl", "anm_reload_w_gl" }, true, GetState());
	}
	else
