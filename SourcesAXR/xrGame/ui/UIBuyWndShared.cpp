...
const u32 CItemMgr::GetItemIdx(const shared_str& sect_name) const
{
	...
	{
#ifdef DEBUG
		LogInfo("item not found in registry [%s]", sect_name.c_str());
#endif // DEBUG
		return u32(-1);
	}
...
void CItemMgr::Dump() const
{
	...
	for(;it!=it_e;++it)
	{
		...
		LogInfo("[%s] slot=[%d] cost= %d,%d,%d,%d,%d",it->first.c_str(),val.slot_idx,val.cost[0],val.cost[1],val.cost[2],val.cost[3],val.cost[4]);
	}
