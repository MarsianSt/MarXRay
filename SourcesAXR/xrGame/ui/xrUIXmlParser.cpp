...
CUIXml::~CUIXml()
{
	...
	for(;_it!=dbg_list_xmls.end();++_it){
		...
		if( (*_it).num==m_dbg_id && (*_it).closed){
			LogInfo("--XML [%d] already deleted", m_dbg_id);
			bOK = true;
		}
	}
	if(!bOK)
		LogInfo("CUIXml::~CUIXml.[%d] cannot find xml in list", m_dbg_id);
#endif
}
