...
void CUIHudStatesWnd::SetAmmoIcon(const shared_str& sect_name)
{
	...
	Frect texture_rect;
	texture_rect.x1					= pSettings->r_float(sect_name, "inv_grid_x")		* UI().inv_grid_kx();
	texture_rect.y1					= pSettings->r_float(sect_name, "inv_grid_y")		* UI().inv_grid_kx();
	texture_rect.x2					= pSettings->r_float(sect_name, "inv_grid_width")	* UI().inv_grid_kx();
	texture_rect.y2					= pSettings->r_float(sect_name, "inv_grid_height")	* UI().inv_grid_kx();
	...
	if (texture_rect.width() > 2.01f * UI().inv_grid())
	{
		w = UI().inv_grid() * 1.5f;
	}
	...
	auto CreateIndicator = [&](CUIMultiElement* ind_name, float state, float state_crit, float state_start, bool reverse)
		{
			if (ind_name)
			{
				...
				if (ind_name == _ind_weapon_broken)
				{
					...
					if (slot == KNIFE_SLOT || slot == PISTOL_SLOT || slot == INV_SLOT_2 || slot == INV_SLOT_3)
