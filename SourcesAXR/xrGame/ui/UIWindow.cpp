...
CUIWindow::~CUIWindow()
{
	...
	for(;_it!=dbg_list_wnds.end();++_it){
		...
		if( (*_it).num==m_dbg_id && (*_it).closed){
			LogInfo("--CUIWindow [%d] already deleted", m_dbg_id);
			bOK = true;
		}
	}
	if(!bOK)
		LogInfo("CUIWindow::~CUIWindow.[%d] cannot find window in list", m_dbg_id);
#endif
}
