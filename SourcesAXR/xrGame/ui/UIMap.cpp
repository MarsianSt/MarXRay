...
void CUICustomMap::Initialize(shared_str name, LPCSTR sh_name)
{
	...
	{
		LogInfo("! default LevelMap used for level[%s]",name.c_str());
...
void CUILevelMap::Init_internal	(const shared_str& name, CInifile& pLtx, const shared_str& sect_name, LPCSTR sh_name)
{
	...
	{
		LogInfo(" --incorrect global rect definition for map [%s]  kw=%f kh=%f",*MapName(),kw,kh);
		LogInfo(" --try x2=%f or  y2=%f",m_GlobalRect.x1+kh*BoundRect().width(), m_GlobalRect.y1+kw*BoundRect().height());
	}
#endif
}
