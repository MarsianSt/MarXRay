	...
	for(;it!=m_GameMaps.end();++it){
		...
		for(it2=it; it2!=m_GameMaps.end();++it2){
			...
			if(l->GlobalRect().intersected(l2->GlobalRect())){
				LogInfo(" --error-incorrect map definition global rect of map [%s] intersects with [%s]", *l->MapName(), *l2->MapName());
			}
		}
		if(FALSE == l->GlobalRect().intersected(GlobalMap()->BoundRect())){
			LogInfo(" --error-incorrect map definition map [%s] places outside global map", *l->MapName());
		}
...
u16 CUIMapWnd::GetIdxByName(const shared_str& map_name)
{
	...
	if(it==m_GameMaps.end()){	
		LogInfo("~ Level Map '%s' not registered",map_name.c_str());
		return						u16(-1);
	}
