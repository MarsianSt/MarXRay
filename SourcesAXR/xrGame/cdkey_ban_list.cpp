...
void cdkey_ban_list::load()
{
		...
		ie = banlist.end(); i != ie; ++i)
	{
		...
		{
			LogInfo("! ERROR: load [%s] ban item section", (*i)->Name.size() > 0 ? (*i)->Name.c_str() : "");
...
bool cdkey_ban_list::is_player_banned(char const * hexstr_digest, shared_str & buy_who)
{
	...
	LogInfo("* checking for ban player [%s]", hexstr_digest);
	
	erase_expired_ban_items();
	for (ban_list_t::iterator i = m_ban_list.begin(),
		ie = m_ban_list.end(); i != ie; ++i)
	{
//		LogInfo("* comparing with cheater [%s]", (*i)->client_hexstr_digest.c_str());
		if (!xr_strcmp((*i)->client_hexstr_digest, hexstr_digest))
		{
			LogInfo("* found banned client [%s] by admin [%s]", 
				hexstr_digest,
...
void cdkey_ban_list::ban_player(xrClientData const * player_data, s32 end_time_sec, xrClientData const * admin)
{
	...
	{
		LogInfo("! ERROR: Can't ban client without unique digest, try to ban by IP address.");
		xr_delete(tmp_client);
		return;
	}
...
void cdkey_ban_list::unban_player_by_index(size_t const index)
{
	...
	{
		LogInfo("! ERROR: bad player index");
		return;
	}
...
void cdkey_ban_list::print_ban_list(char const * filter_string)
{
	...
	LogInfo("- ----banned players list begin-------");
	...
	LogInfo("- ----banned players list end-------");
}
...
bool cdkey_ban_list::banned_client::load(CInifile* ini, shared_str const & name_sect)
{
	...
	if (ban_end_time == 0)
	{
		LogInfo("! ERROR bad ban_end_time in section [%s]", name_sect.c_str());
		return false;
	}
...
void cdkey_ban_list::erase_expired_ban_items()
{
	struct expire_searcher_predicate
	{
		...
		bool operator()(banned_client* bclient)
		{
			...
			{
				LogInfo("- Ban of %s is expired", bclient->client_name.c_str());
				xr_delete(bclient);
				return true;
			}
			return false;
