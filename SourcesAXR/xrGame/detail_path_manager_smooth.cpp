...
#ifdef DEBUG
		if (m>=10000) {
			LogInfo("! [position.radius=%f],[angle=%f],[m=%d]",position.radius,angle,m);
			VERIFY		(m<10000);
		}
#endif
		n				= !m ? 1 : m;
...
{
//	LogInfo("[%6d][%s] started to build detail path",Device.dwFrame,*m_restricted_object->object().cName());
	...
	if (m_restricted_object) {
		...
		if (!((alvi && asp) || (!asp && !alvi))) {
			LogInfo("! vertex [%d], position [%f][%f][%f], alvi[%c], asp[%c]",start.vertex_id,VPUSH(start_pos), alvi ? '+' : '-', asp ? '+' : '-');
		}
	...
	STOP_PROFILE;
//	LogInfo("[%6d][%s] build_detail_path [%d][%d][%d]",Device.dwFrame,*m_restricted_object->object().cName(),path().size(),curr_travel_point_index(),last_patrol_point());
}
