...
{	
	...
	{
		LogInfo("! Unable to find %s in registry", REGISTRY_PATH);
		return false;
	}

	if (!hKey) 
	{
		LogInfo("! Unable to find %s entry in registry", REGISTRY_PATH); 
		return false;
	}
	...
	default:
		{
			LogInfo("! Unknown registry data type.");
			return false;
		}break;
	};	
	...
	if (res != ERROR_SUCCESS)
	{
		LogInfo("! Unable to find %s entry in registry", rKeyName); 
		return false;
	}
