				...
				if (actor && IsGameTypeSingle()/**/||stalker/**/){
					...
					if (Fsphere::rpNone!=S.intersect_full(bullet->bullet_pos, bullet->dir, dist))
					{
						...
						if (actor){
							...
							CAI_Stalker				*stalker_obj = smart_cast<CAI_Stalker*>(initiator);
							if (stalker_obj)
								hpf					= stalker_obj->SpecificCharacter().hit_probability_factor();
...
//		LogInfo("Hit sended: %d[%d,%d]", Hit.whoID, Hit.weaponID, Hit.BulletID);
							    ...
							    collide::rq_result& R, u16 target_material, Fvector& hit_normal )
{
	...
	else
	{
		...
		if ( DOT( hit_normal, bullet->dir ) < 0 )
		{
			if ( bullet->density_mode )
			{
//				LogInfo("%s", "WARNING: Material in material found while bullet tracing. Incorrect behaviour of shooting is possible.");
			}
