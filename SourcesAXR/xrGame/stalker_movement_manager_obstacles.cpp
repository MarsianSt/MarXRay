...
//		LogInfo( "%6d stalker %s waits for the some door to be open/closed", Device.dwTimeGlobal, object().cName().c_str() );
		float						desirable_speed = old_desirable_speed();
		set_desirable_speed			(0.f);
...
//	LogInfo( "%6d stalker %s is going", Device.dwTimeGlobal, object().cName().c_str() );
