...
void	game_sv_ArtefactHunt::assign_RP(CSE_Abstract* E, game_PlayerState* ps_who)
{
	...
	u32		Team				= RP_2_Use(E);
#ifdef DEBUG
	LogInfo("--- ArtefactHunt RPoint for %s uses team %d", ps_who->getName(), Team);
#endif // #ifdef DEBUG
	...
	{
#ifdef DEBUG
		LogInfo("--- No free Arthfacthunt rpoints found, trying to find rpoints with enemies!");
#endif // #ifdef DEBUG
...
#ifndef MASTER_GOLD
	LogInfo("---select artefact RPoint [%d]", ID);
