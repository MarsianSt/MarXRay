...
#ifdef DEBUG
	if (object().agent_manager().get_member().combat_members().empty())
		LogInfo("! I am in combat, but there is no combat members at all (including me), npc[%s],team[%d],squad[%d],group[%d]",
			*object().cName(),
			object().g_Team(),
			object().g_Squad(),
			object().g_Group()
...
void CStalkerActionCombatBase::setup_cover	(CCoverPoint const& cover)
{
	if (cover.m_is_smart_cover) {
		object().get_movement().target_params().cover_id(static_cast<smart_cover::cover const&>(cover).id());
		return;
	}

	object().get_movement().set_level_dest_vertex		(cover.level_vertex_id());
	object().get_movement().set_desired_position		(&cover.position());
}