...
void __stdcall account_manager::only_log_creation_cb(bool success, char const * descr)
{
	if (success)
	{
		LogInfo("* New profile has created successfully !");
		return;
	}
	LogInfo("! GameSpy account creation ERROR: %s", descr ? descr : "unknown");
}
...
void	__stdcall account_manager::only_log_profdel_cb(bool success, char const * descr)
{
	if (success)
	{
		LogInfo("* Profile has been deleted. And logged out.");
		return;
	}
	LogInfo("! FAILED to delete GameSpy profile: %s", descr ? descr : "unknown");
}
												...
												char const * description)
{
	if (profiles_count)
	{
		LogInfo("- GameSpy account profiles:");
	...
	LogInfo("- No GameSpy account profiles found: %s", description ? description : "unknown error");
}
void __stdcall account_manager::only_log_suggestions(u32 const profiles_count,
													 char const * description)
{
	if (description)
		LogInfo("- GameSpy suggested unique nicks: %s", description);
													 ...
													 char const * user_name)
{
	if (!found)
	{
		LogInfo("- Such email not found: %s", user_name ? user_name : "");
		return;
	}
	LogInfo("- Found email, user nick is: %s", user_name);
}
...
bool account_manager::verify_nick(char const * nick)
{
	if (!nick)
	{
		LogInfo("! ERROR: nick name is empty");
...
bool account_manager::verify_unique_nick	(char const * unick)
{
	...
	if (!unick)
	{
		LogInfo("! ERROR: unique nick name is empty");
	...
	{
		LogInfo("! ERROR: unique nick name is too short (must be greater that 2 character)");
		m_verifyer_error = "mp_gp_unique_nick_too_short";
		return false;
	}
	if (unick_length >= GP_UNIQUENICK_LEN)
	{
		LogInfo("! ERROR: nick name is too big");
	...
	{
		LogInfo("! ERROR: first symbol is invalid");
	...
	if (strchr(unick, ' '))
	{
		LogInfo("! ERROR: unique nick can't contain spaces");
...
bool account_manager::verify_email	(char const * email)
{
	...
	{
		LogInfo("! ERROR: email is too big");
...
bool account_manager::verify_password	(char const * pass)
{
	if (!pass)
	{
		LogInfo("! ERROR: password is empty");
	...
	if (pass_length <= 1)
	{
		LogInfo("! ERROR: password is too small, must be greater than 1 symbol");
		m_verifyer_error = "mp_gp_password_is_too_small";
		return false;
	}
	if (pass_length >= GP_PASSWORD_LEN)
	{
		LogInfo("! ERROR: password is too big");
