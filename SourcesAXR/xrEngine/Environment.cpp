	...
	m_ambients_config		(0)
{
	...
	m_wind_blast_strength		= 0.f;
	m_wind_blast_direction.set(1.f,0.f,0.f);
...
void CEnvironment::OnFrame()
{
	...
	float WindVel = _max(CurrentEnv->wind_velocity * 4, ps_ssfx_wind_trees.w * 1000);

	// Limit min at 200 to avoid slow-mo at extremly low speed.
	WindVel = _max(WindVel, bWeatherWindInfluenceKoef * 450) * 0.001f;
