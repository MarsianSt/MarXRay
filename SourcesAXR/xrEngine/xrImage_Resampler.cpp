...
void	imf_Process	(u32* dstI, u32 dstW, u32 dstH, u32* srcI, u32 srcW, u32 srcH, EIMF_Type FILTER)
{
	...
	if(xscale < 1.0) {
		try	{
			...
			LogInfo("imf_Process::3 (xscale<1.0)");
		};
	} else {
		try	{
			for(i = 0; i < dst.xsize; ++i)
	...
	if(yscale < 1.0) {
		...
		} catch (...) {	LogInfo("imf_Process::8 (yscale<1.0)");	};
	} else {
		try	{
			for(i = 0; i < dst.ysize; ++i)
			{
