...
template<class C>	void mdump(C c)
{
	...
	for (C::iterator I=c.begin(); I!=c.end(); I++)
		LogInfo("*        : %3d: %s",I->second->dwReference, I->second->cName.c_str());
}
...
{
	LogInfo("* RM_Dump: textures  : %d",		m_textures.size());		if(!bBrief) mdump(m_textures);
	LogInfo("* RM_Dump: rtargets  : %d",		m_rtargets.size());		if(!bBrief) mdump(m_rtargets);
//	DX10 cut 	LogInfo("* RM_Dump: rtargetsc : %d",		m_rtargets_c.size());	if(!bBrief) mdump(m_rtargets_c);
	LogInfo("* RM_Dump: vs        : %d",		m_vs.size());			if(!bBrief) mdump(m_vs);
	LogInfo("* RM_Dump: ps        : %d",		m_ps.size());			if(!bBrief) mdump(m_ps);
	LogInfo("* RM_Dump: dcl       : %d",		v_declarations.size());
	LogInfo("* RM_Dump: states    : %d",		v_states.size());
	LogInfo("* RM_Dump: tex_list  : %d",		lst_textures.size());
	LogInfo("* RM_Dump: matrices  : %d",		lst_matrices.size());
	LogInfo("* RM_Dump: lst_constants: %d",	lst_constants.size());
	LogInfo("* RM_Dump: v_passes  : %d",		v_passes.size());
	LogInfo("* RM_Dump: v_elements: %d",		v_elements.size());
	LogInfo("* RM_Dump: v_shaders : %d",		v_shaders.size());
}
