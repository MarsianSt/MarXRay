...
#elif BX_PLATFORM_WINDOWS
		// Use modern thread naming API (Windows 10+)
		HMODULE kernel32 = GetModuleHandleA("kernel32.dll");
		if (kernel32)
		{
			auto setThreadDescription = (HRESULT(WINAPI*)(HANDLE, PCWSTR))GetProcAddress(kernel32, "SetThreadDescription");
			if (setThreadDescription)
			{
				int len = MultiByteToWideChar(CP_ACP, 0, m_name.getCPtr(), -1, NULL, 0);
				if (len > 0)
				{
					wchar_t* wname = (wchar_t*)_alloca(len * sizeof(wchar_t));
					MultiByteToWideChar(CP_ACP, 0, m_name.getCPtr(), -1, wname, len);
					setThreadDescription(ti->m_handle, wname);
				}
			}
		}
#endif // BX_PLATFORM_
