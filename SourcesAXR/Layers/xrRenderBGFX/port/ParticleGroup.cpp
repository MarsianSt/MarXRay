...
namespace
{
CParticleEffect* bgfxParticle_CreatePE(LPCSTR name)
{
    return static_cast<CParticleEffect*>(::Render->model_CreateParticles(name));
}
}
...
BOOL CPGDef::Load(IReader& F)
{
    ...
    if (version!=PGD_VERSION){
		LogInfo("%s", "!Unsupported PG version. Load failed.");
    	return FALSE;
    }
...
{
    CParticleEffect*C		= bgfxParticle_CreatePE(eff_name);
...
{
    CParticleEffect*C			= bgfxParticle_CreatePE(nm);
	C->SetHudMode				(emitter->GetHudMode());
    if(!C->IsLooped())
	{
        Fmatrix M; 				M.identity();
...
BOOL CParticleGroup::Compile(CPGDef* def)
{
    ...
    if (m_Def){
        ...
        for (CPGDef::EffectVec::const_iterator e_it=m_Def->m_Effects.begin(); e_it!=m_Def->m_Effects.end(); e_it++){
        	CParticleEffect* eff = bgfxParticle_CreatePE(*(*e_it)->m_EffectName);
