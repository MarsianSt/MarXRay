...
void CUICompassPanel::AddPoints()
{
	...
	for (const auto& map_loc : Level().MapManager().Locations())
	{
		...
		bool is_quest = (strstr(str, "primary_task") == str || strstr(str, "storyline_task") == str || strstr(str, "secondary_task") == str);
		...
		if (enabled)
		{
			...
			if (is_quest)
			{
				...
				CGameTask* main_objective = Level().GameTaskManager().ActiveTask(eTaskTypeStoryline);
				if (main_objective)
				{
					u16 target_id = main_objective->GetMapObjectID();
					if (target_id != u16(-1))
						main_target_obj = ai().alife().objects().object(target_id, true);
				}

				CGameTask* additional_objective = Level().GameTaskManager().ActiveTask(eTaskTypeAdditional);
				if (additional_objective)
				{
					u16 target_id = additional_objective->GetMapObjectID();
					if (target_id != u16(-1))
						additional_target_obj = ai().alife().objects().object(target_id, true);
				}
				...
				if (main_objective && task->m_ID == main_objective->m_ID)
				{
					alife_obj = main_target_obj;
					is_active = true;
				}
				else if (additional_objective && task->m_ID == additional_objective->m_ID)
				{
					alife_obj = additional_target_obj;
					is_active = true;
				}
